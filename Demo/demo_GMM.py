...
# -*- coding: utf-8 -*-
"""
Created on Fri Jun 21 23:25:57 2019
...
#                    frames.append(gray)
                    ...
                    cv.imshow('Original video GMM', gray) 
                    cv.imshow('Background detected GMM', bg)
                    cv.imshow('Foreground detected GMM', fg) 
...
#    daytimeDetection(means)    
    ...
    gauss.plotAPIcomparison(APIs, bg_APIs, 'GMM')
...
def showComputation(bg, fg, frames):
    ...
    while(cycle):
        ...
        cv.imshow('GMM Backgroung', bg[i])
        cv.imshow('GMM Foregroung', fg[i])
        cv.imshow('GMM Original frame', frames[i])
